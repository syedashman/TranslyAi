...
from app.services.meeting_jobs import create_job, get_job, update_job
...
class LiveSession:
    def __init__(self, job_id: str, user_id: Optional[str], chat_id: str):
        ...
        self.stage = "waiting"  # public: waiting | live | host_disconnected | finalizing | translating | summarizing | completed | failed
        self.outcome: Optional[str] = None  # None | completed | failed
    ...
    async def broadcast(self, event: dict, *, host: bool = True) -> None:
        """The same event, in the same order, to the host and every viewer. A slow or dead viewer is dropped and can
        never delay the others (each delivery is bounded) or the pipeline."""
        targets = list(self.viewers.items())
        results = await asyncio.gather(*(self._deliver(ws, lock, event) for ws, lock in targets), return_exceptions=True)
        dropped = [ws for (ws, _), ok in zip(targets, results) if ok is not True]
        for ws in dropped:
    ...
    def feed_audio(self, data: bytes) -> None:
        if self.state != "active" or self._stt is None:
            return
